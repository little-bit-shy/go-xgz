...
func NewPool(c *Config, options ...DialOption) (p *Pool) {
	...
	// new pool
	p1.New = func(ctx context.Context) (n io.Closer, err error) {
		if t, ok := trace.FromContext(ctx); ok {
			var internalTags []trace.Tag
			internalTags = append(internalTags, trace.TagString(trace.TagComponent, _traceComponentName))
			internalTags = append(internalTags, trace.TagString(trace.TagPeerService, _tracePeerService))
			internalTags = append(internalTags, trace.TagString(trace.TagSpanKind, _traceSpanKind))

			t = t.Fork(_family, "redis:connect")
			t.SetTag(trace.String(trace.TagAddress, fmt.Sprintf("%+v", c.Addr)))
			defer t.Finish(&err)
		}
		var con Conn
		con, err = Dial(c.Proto, c.Addr, ops...)
		if err != nil {
			return
		}
		n = &traceConn{
