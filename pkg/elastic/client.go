...
import (
	...
	"github.com/little-bit-shy/go-xgz/pkg/elastic/config"
)
...
type Client struct {
	...
	trace func(error, time.Time, *Request, *Response)
}
...
// SetTracer sets the tracher for informational messages, e.g. requests
// and their response
func SetTracer(tracer func(error, time.Time, *Request, *Response)) ClientOptionFunc {
	return func(c *Client) error {
		c.trace = tracer
		return nil
	}
...
func (c *Client) PerformRequest(ctx context.Context, opt PerformRequestOptions) (*Response, error) {
	...
	for {
		...
		if ok {
			var bodyByte []byte
			bodyByte, err = json.Marshal(opt.Body)
			...
			t = t.Fork(_family, "Elastic:"+req.Method)
			t.SetTag(trace.String(trace.TagPeerHostname, req.RemoteAddr),
				trace.String(trace.TagComment, fmt.Sprintf("%s", req.URL.RequestURI())),
				trace.String(trace.TagAnnotation, fmt.Sprintf("%+v", body)))
		}
