...
func (db *conn) exec(c context.Context, query string, args ...interface{}) (res sql.Result, err error) {
	now := time.Now()
	logSql := fmt.Sprintf(strings.Replace(query, "?", "%v", -1), args...)
	defer slowLog(logSql, now)
	if t, ok := trace.FromContext(c); ok {
		t = tracer(t, "exec")
		t.SetTag(trace.String(trace.TagAddress, db.addr), trace.String(trace.TagComment, logSql))
		defer t.Finish(&err)
	}
	if err = db.breaker.Allow(); err != nil {
...
func (s *Stmt) Exec(c context.Context, args ...interface{}) (res sql.Result, err error) {
	...
	now := time.Now()
	logSql := fmt.Sprintf(strings.Replace(s.query, "?", "%v", -1), args...)
	defer slowLog(logSql, now)
	if s.tx {
		if s.t != nil {
			s.t.SetTag(trace.String(trace.TagAnnotation, logSql))
		}
	} else if t, ok := trace.FromContext(c); ok {
		t = tracer(t, "exec")
		t.SetTag(trace.String(trace.TagAddress, s.db.addr), trace.String(trace.TagComment, logSql))
		defer t.Finish(&err)
	}
...
func (tx *Tx) Exec(query string, args ...interface{}) (res sql.Result, err error) {
	...
	defer slowLog(logSql, now)
	if tx.t != nil {
		tx.l = append(tx.l, logSql)
		tx.t.SetTag(trace.String(trace.TagComment, strings.Join(tx.l, "\r\n")))
	}
